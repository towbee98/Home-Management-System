...
const stateArray = Object.keys(lgaList);
if (country) {
  country.addEventListener("change", () => {
    if (country.selectedOptions[0].textContent === "Nigeria") {
...
if (state) {
  state.addEventListener("change", (e) => {
    if (Object.keys(lgaList).includes(state.selectedOptions[0].textContent)) {
      lgaList[state.selectedOptions[0].textContent].forEach((el) => {
        const tag = document.createElement("option");
        tag.setAttribute("value", el);
        tag.append(`${el}`);
        lga.appendChild(tag);
      });
    }
